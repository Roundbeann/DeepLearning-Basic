...
def build_curpus(train_text):
    ...
    index2onehot = numpy.eye(wordsize,dtype=np.float32)
    return word2index,index2onehot
class OHDataset(Dataset):
    def __init__(self,texts,labels,word2index,index2onehot,max_len):
        self.texts = texts
        self.labels = labels
        self.word2index = word2index
        self.index2onehot = index2onehot
    ...
    def __getitem__(self, index):
        ...
        if len(text)>=self.max_len:
            ...
            for i in text:
                try:
                    text2onehot.append(self.index2onehot[self.word2index[i]])
                except:
                    text2onehot.append(self.index2onehot[self.word2index['<UNK>']])
            ...
            for i in range(pad_num):
                text2onehot.append(self.index2onehot[self.word2index['<PAD>']])
            text2onehot = np.array(text2onehot)
...
class OHModel(nn.Module):
    def __init__(self,max_len,word_count_in_dict,hidden_num,cls_num):
        super().__init__()
        self.linear1 = nn.Linear(word_count_in_dict,hidden_num)
        self.active = nn.ReLU()
        self.flatten = nn.Flatten()
        self.linear2 = nn.Linear(max_len * hidden_num,cls_num)
        self.cross_loss = nn.CrossEntropyLoss()
    def forward(self,text2onehot,labels = None):
        hidden = self.linear1(text2onehot)
...
def test_file():
    global model,device,word2index,index2onehot

    test_text,test_label = read_data("test.txt")
    assert len(test_text) == len(test_label)
    test_onehot_dataset = OHDataset(test_text, test_label, word2index, index2onehot, max_len)
...
if __name__ == "__main__":
    ...
    word2index,index2onehot = build_curpus(train_text)
    word_count_in_dict = len(word2index)

    model = OHModel(max_len,word_count_in_dict,hidden_num,cls_num).to(device)
    optim = torch.optim.AdamW(model.parameters(),lr = lr)

    train_onehot_dataset = OHDataset(train_text,train_label,word2index,index2onehot,max_len)
    # a = trainOnehotDataset[0]
    train_dataloader = DataLoader(train_onehot_dataset,batch_size)

    dev_onehot_dataset = OHDataset(dev_text,dev_label,word2index,index2onehot,max_len)
    # a = trainOnehotDataset[0]
