...
class PoemGeneration(nn.Module):
    ...
    def forward(self,pre_idx,later_idx=None,h_0 = None):
        batch_size ,sent_len = pre_idx.shape
        embed_pre_idx = self.embedding(pre_idx)
        output1,output2 = self.lstm(embed_pre_idx,hx = h_0)
        ...
        if later_idx is not None:
            ...
            return torch.argmax(pre,dim=-1).reshape(-1).detach().to("cpu").numpy().tolist()[-1],output2
        pass
...
def generate():
    ...
    while True:
        ...
        result += word
        for i in range(23):
            sent_idx = [word2index[c] for c in result][-1]
            word_index = torch.tensor([[sent_idx]])
            pre,h0 = model.forward(word_index,h_0=h0)
            if i == 22:
                print()
...
if __name__ == "__main__":
    ...
    for e in range(epoch):
        ...
        for pre_idx, later_idx in trainDataLoader:
            loss = model.forward(pre_idx,later_idx =later_idx )
            loss.backward()
            optim.step()
            optim.zero_grad()
