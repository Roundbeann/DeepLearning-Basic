...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.RNN(embedLen,hiddenNum,batch_first=True)
        self.decoder = nn.RNN(embedLen,hiddenNum,batch_first=True)
    ...
    def forward(self, batchEng,batchChi=None):
        ...
        # W2 128 * 36
        # pre 4 * 7 * 36
        engEmbed = self.embeddingEng(batchEng)
        # encoder 是什么=>每句话的句向量
        _, eachSentFeat = self.encoder(engEmbed)
        if batchChi is not None:
            chiEmbed = self.embeddingChi(batchChi[:,:-1])
            # decoder 是什么=>每个中文对应的字向量
            chiOutputFeat,_ = self.decoder(chiEmbed,eachSentFeat)
            pre = self.cls(chiOutputFeat)
            loss = self.loss(pre.reshape(pre.shape[0]*pre.shape[1],-1),batchChi[:,1:].reshape(-1))
            print(index2eng[batchEng[0].numpy()],index2chi[batchChi[0].numpy()],index2chi[torch.argmax(pre[0],dim=-1).numpy()])
            print()
            return loss
        else :
            res = [2]
            for i in range(4):
                ...
                chiEmbed = self.embeddingChi(batchChi)
                chiOutputFeat, eachSentFeat = self.decoder(chiEmbed, eachSentFeat)
                pre = torch.argmax(self.cls(chiOutputFeat).reshape(-1),dim=-1)
                res.append(int(pre))
                pass
