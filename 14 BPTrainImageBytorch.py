...
class MyModel(nn.Module):
    def __init__(self,inputSize,hiddenNum,classNum):
        super().__init__()
        self.linear1 = nn.Linear(inputSize,hiddenNum)
        self.sigmoid = nn.Sigmoid()
        self.linear2 = nn.Linear(hiddenNum,classNum)
        self.crossLoss = nn.CrossEntropyLoss()

    def forward(self,X, label = None):
        hidden = self.linear1(torch.tensor(X,dtype=torch.float32))
        sig_hidden = self.sigmoid(hidden)
        p = self.linear2(sig_hidden)
        if label is not None:
            loss =self.crossLoss(p,torch.tensor(label,dtype=torch.int64))
            return loss
        return p
...
if __name__ == "__main__":
    ...
    print("over")
    train_label_oh  = make_one_hot(train_label)
    test_label_oh  = make_one_hot(test_label)
    ...
    class_num = len(set(test_label))

    model = MyModel(input_size,hidden_num,class_num)
    ...
    for x, start, end in DataLoader(test_data, batch_size):
        pre = model(x)
        label = torch.tensor(test_label[start:end],dtype=torch.int64)
        lossFun = nn.CrossEntropyLoss()
        loss = lossFun(pre ,label)

        print(f"avg_loss:{loss}")
        plt.imshow(model.state_dict()['linear1.weight'].numpy())
        plt.show()
        for i in range(len(model.state_dict()['linear2.weight'])):
            plt.imshow(model.state_dict()['linear2.weight'][i,:].reshape(16,16))
            plt.show()
