...
class Positional(nn.Module):
    def __init__(self,embedding_num):
        super().__init__()
        self.position = torch.normal(0,1,size=(max_len,1),device=device)
        self.position = torch.from_numpy(numpy.repeat(self.position.detach().to("cpu").numpy(),embedding_num,axis=1)).to(device)
