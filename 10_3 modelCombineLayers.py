...
if __name__ == "__main__":
    ...
    # # sigmoid2_layer = Sigmoid()
    # # linear3_layer = Linear(10, 10)
    ...
    for X, start, end in DataLoader(test_data, batch_size):
        ...
        print([i.argmax() for i in x[:20]])
        accuracy = (np.array([i.argmax() for i in label]) == np.array([i.argmax() for i in x])).astype(int).mean()
