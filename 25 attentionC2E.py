...
English = ["cat", "rice", "banana", "pink", "apple", "orange", "peach", "blue", "black", "yellow", "green", "red",
           "white"]
# Chinese = ["蓝色", "黑色", "黄色", "绿色", "红色", "白色"]
# English = ["blue", "black", "yellow", "green", "red","white"]
#

def getIndexAndDict(Chinese, English):
    chi2index = {"UNK": 1, "PAD": 0}
    eng2index = {"UNK": 1, "PAD": 0, "STA": 2, "END": 3}
...
class transDataset(Dataset):
    ...
    def __getitem__(self, index):
        engIndex = torch.tensor([2] + self.engIndex[index][:engMaxLen] + [3] +(engMaxLen - len(self.engIndex[index])) * [0])
        chiIndex = torch.tensor(
             self.chiIndex[index][:chiMaxLen] + (chiMaxLen - len(self.chiIndex[index])) * [0])
        return engIndex, chiIndex, len(self.engIndex[index])+2, len(self.chiIndex[index])
...
class Encoder(nn.Module):
    ...
    def forward(self, batchChiEmb, chiLen):
        pack = pack_padded_sequence(batchChiEmb, chiLen, batch_first=True, enforce_sorted=False)
...
class Decoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = nn.RNN(embedLen,hiddenNum,batch_first=True)
        self.cls = nn.Linear(hiddenNum,engDictLen)
    def forward(self,batchEngEmb,chiHistory):
        output1, engHistory = self.model(batchEngEmb, chiHistory)
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.att_lin1 = nn.Linear(embedLen,embedLen)
        self.tanh = nn.Tanh()
        self.att_lin2 = nn.Linear(embedLen,1)
    ...
    def forward(self, batchChi, chiLen, batchEng=None):
        batchChiEmb = self.embeddingChi(batchChi)

        batchChiEmbATT = self.att_lin1(batchChiEmb)
        batchChiEmbATT = self.tanh(batchChiEmbATT)
        batchChiEmbATT = self.att_lin2(batchChiEmbATT)

        batchChiEmb = batchChiEmb * batchChiEmbATT
        eachSentFeat = self.encoder(batchChiEmb, chiLen)
        if batchEng is not None:
            batchEngEmb = self.embeddingEng(batchEng[:, :-1])
            pre, history = self.decoder(batchEngEmb, eachSentFeat)
            loss = self.loss(pre.reshape(pre.shape[0] * pre.shape[1], -1), batchEng[:, 1:].reshape(-1))
            return loss
        else:
            res = [2]
            for i in range(4):
...
if __name__ == "__main__":
    ...
    for e in trange(epoch):
        ...
        for batchEng, batchChi, engLen, chiLen in transLoader:
            loss = model.forward(batchChi, chiLen, batchEng)
            loss.backward()
            opt.step()
            opt.zero_grad()
        print(loss)
