...
class PoemGeneration(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.RNN(embedding_len,hidden_num,batch_first=True)
        self.cls = nn.Linear(hidden_num,dict_len)
        self.loss = nn.CrossEntropyLoss()
    def forward(self,pre_idx,later_idx=None,h = None):
        batch_size ,sent_len = pre_idx.shape
        embed_pre_idx = self.embedding(pre_idx)
        output1,output2 = self.rnn(embed_pre_idx,h)
        ...
        if later_idx is not None:
            ...
            return torch.argmax(pre,dim=-1).reshape(-1).detach().to("cpu").numpy().tolist()[-1],output1
        pass
...
def generate():
    ...
    while True:
        ...
        for i in range(23):
            ...
            pre,h = model.forward(word_index,h=h)
