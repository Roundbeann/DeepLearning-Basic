...
class TextDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(text)>=self.max_len:
            ...
            text2onehot = np.array(text2onehot)
        return text2onehot,label,text_len
    def __len__(self):
        return len(self.labels)

class TransformerClassifier(nn.Module):
    def __init__(self,embedding_num,cls_num,device):
        super().__init__()

        self.transformer = TransformerEncoder(device,embedding_num=embedding_num)
        self.classifier = nn.Linear(embedding_num,cls_num)
        self.loss_fun = nn.CrossEntropyLoss()

    def forward(self,text2embedding,datas_len,labels = None):
        # text2embedding [batch_size, max_len, embed_len]
        out = self.transformer(text2embedding,datas_len)
        out_ = out[:,0,:]
        p = self.classifier(out_)

        if labels is not None:
            loss = self.loss_fun(p,labels)
            return loss
...
def test_file():
    ...
    test_onehot_dataset = TextDataset(test_text, test_label, word2index, index2embedding, max_len)
    # a = trainOnehotDataset[0]
    test_dataloader = DataLoader(test_onehot_dataset, 2000)

    result = []
    for text,label,text_len in test_dataloader:
        text = text.to(device)
...
if __name__ == "__main__":
    ...
    model = TransformerClassifier(embedding_num,cls_num,device).to(device)
    optim = torch.optim.AdamW(model.parameters(),lr = lr)

    train_onehot_dataset = TextDataset(train_text,train_label,word2index,index2embedding,max_len)
    # a = trainOnehotDataset[0]
    train_dataloader = DataLoader(train_onehot_dataset,batch_size)

    dev_onehot_dataset = TextDataset(dev_text,dev_label,word2index,index2embedding,max_len)
    # a = trainOnehotDataset[0]
    dev_dataloader = DataLoader(dev_onehot_dataset,1000)


    for e in trange(epoch):
        for texts,labels ,text_len in train_dataloader:
            texts = texts.to(device)
            labels = labels.to(device)
            loss = model(texts,text_len,labels)
            loss.backward()
            optim.step()
            optim.zero_grad()
        print(f"\nloss{loss:.2f}")
        ...
        result_dev = []
        for texts,labels,text_len in dev_dataloader:
            texts = texts.to(device)
            labels = labels.to(device)
            model(texts,text_len)
            pre = model.predict
