...
class OHDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(text)>=self.max_len:
            ...
            for i in text:
                try:
                    text2onehot.append(self.index2embedding[self.word2index[i]])
                except:
                    text2onehot.append(self.index2embedding[self.word2index['<UNK>']])
            text2onehot = np.array(text2onehot)
            ...
            for i in range(pad_num):
                text2onehot.append(self.index2embedding[self.word2index['<PAD>']])
            text2onehot = np.array(text2onehot)
...
class Positional(nn.Module):
    def __init__(self,embedding_num):
        super().__init__()
        self.position = torch.zeros(size=(max_len,embedding_num),device=device,requires_grad=False)
        t = torch.arange(1,max_len+1,dtype=torch.float).unsqueeze(1).to(device)
        k_2i = torch.arange(0,embedding_num,2).to(device)
        k_2i_plus_1 = k_2i + 1
        w_i = 1/ 10000**(k_2i/embedding_num)
        w_i = w_i.to(device)
        sin_w_t = torch.sin(w_i*t)
        cos_w_t = torch.cos_(w_i * t)
        self.position[:,::2]=sin_w_t
        self.position[:,1::2] = cos_w_t
        print()

    def forward(self,batch_x):
        result = batch_x + self.position
        return  result
...
class Self_Attention(nn.Module):
    ...
    def forward(self,x):
        # x【 batchSize * max_len * embeddingNum】
        # Q、K、V = 【batchSize * max_len * embeddingNum】
        Q = self.W_Q(x)
        K = self.W_K(x)
        V = self.W_V(x)

        # Q @ (K.T) 【max_len * max_len】
        # S 【max_len * max_len】
        S = Q @ (K.transpose(-1,-2))/math.sqrt(embedding_num)
        # score 【max_len * max_len】
        score = self.softmax(S)
        # score【max_len * max_len】
        # V 【max_len * embeddingNum】
        # r 【max_len * embeddingNum】
        r = score @ V
        return r
    # def forward(self,x):
    #     # x【 1 * max_len * embeddingNum】
    #     # Q、K、V = 【max_len * embeddingNum】
    #     Q = self.W_Q(x).squeeze(0)
    #     K = self.W_K(x).squeeze(0)
    #     V = self.W_V(x).squeeze(0)
    #
    #     # Q @ (K.T) 【max_len * max_len】
    #     # S 【max_len * max_len】
    #     S = Q @ (K.T)/math.sqrt(embedding_num)
    #     # score 【max_len * max_len】
    #     score = self.softmax(S)
    #     # score【max_len * max_len】
    #     # V 【max_len * embeddingNum】
...
class OHModel(nn.Module):
    def __init__(self,max_len,embedding_num,hidden_num,cls_num):
        super().__init__()
        self.position = Positional(embedding_num)
        self.self_attention = Self_Attention(embedding_num)
    ...
    def forward(self,text2embedding,labels = None):
        text2embedding = self.position(text2embedding)
        text2embedding = self.self_attention(text2embedding)
